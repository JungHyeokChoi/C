...
void insertMinHeapAH(ArrayHeap* pHeap, HeapNode element) {
	...
	if (pHeap != NULL) {
		...
		if (ret == TRUE) {
			while ((i != 1) && (element.weight < pHeap->pElement[i - 1].weight)) {
				if (element.weight == pHeap->pElement[i - 1].weight) {
	...
	if (pHeap != NULL && pHeap->currentElementCount > 0) {
		...
		*pReturn = pHeap->pElement[1];
		...
		while (child <= pHeap->currentElementCount) {
			...
			if (pTemp->weight <= pHeap->pElement[child].weight) {
				break;
			}
